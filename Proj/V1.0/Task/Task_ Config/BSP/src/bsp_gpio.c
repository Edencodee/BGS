...
void LDSet (void)
{
	GPIO_BOP(LD_PORT) = LD_PIN;
}

void LDClr (void)
{
	GPIO_BC(LD_PORT) = LD_PIN;
}

/* FUN Input function ------------------------------------------------------------ */
fun_state_t FunGetState (void)
{
	return (fun_state_t)gpio_input_bit_get(FUN_PORT, FUN_PIN);
}
...
void GPIO_UserInit(void)
{
	...
	/* configure LD GPIO port (output) */
	rcu_periph_clock_enable(LD_CLK);
	gpio_mode_set(LD_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, LD_PIN);
	GPIO_BC(LD_PORT) = LD_PIN;
