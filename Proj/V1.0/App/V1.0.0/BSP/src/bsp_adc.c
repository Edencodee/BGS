...
/* ADC value buffer -------------------------------------------------- */
__IO uint16_t g_A_Buffer[SAMPLE_POINT] = {0};
__IO uint16_t g_B_Buffer[SAMPLE_POINT] = {0};
/* ------------------------------------------------------------------- */
...
void DMA_Config(void)
{
    ...
    dma_init_struct.memory_addr  = (uint32_t)g_A_Buffer;
...
void ADC_SwitchBuffer(uint16_t *g_adcCaptureBuffer)
{
    /* Swap buffers: minimal latency update */
    dma_channel_disable(ADC_DMA_CHANNEL);
    dma_memory_address_config(ADC_DMA_CHANNEL, (uint32_t)g_adcCaptureBuffer);
    dma_transfer_number_config(ADC_DMA_CHANNEL, SAMPLE_POINT);
    dma_channel_enable(ADC_DMA_CHANNEL);
}
