...
uint8_t IC_CalcCRC(const volatile isl700_config_t *r)
{
	...
	CRC_table(8, r->fuse_check_floating);// SR[50:57]: Fuse check floating (固定值0)
    CRC_table(4, r->fuse_check_low);     // SR[0:3]: Fuse check low (固定值9)
	CRC_table(8, r->pls);                // SR[4:11]: 发射脉冲幅值
	CRC_table(2, r->no_used);            // SR[12:13]: 保留未使用 (固定值1)
	CRC_table(10, r->fine_gain);         // SR[14:23]: 细调增益
	CRC_table(1, r->gain_select);        // SR[24]: 增益选择 (0=x1, 1=x1.5)
	CRC_table(4, r->fuse_check_middle);  // SR[25:28]: Fuse check middle (固定值10)
	CRC_table(4, r->coarse_gain1);       // SR[29:32]: 粗调增益1
	CRC_table(4, r->coarse_gain2);       // SR[33:36]: 粗调增益2
	CRC_table(4, r->sctrl_res_a);        // SR[37:40]: 前级增益a
	CRC_table(4, r->sctrl_res_b);        // SR[41:44]: 前级增益b
	CRC_table(1, r->sctrl_internal);     // SR[45]: 电阻选择 (0=外部, 1=内部)
	CRC_table(4, r->fuse_check_high);    // SR[46:49]: Fuse check high (固定值6)
    crc = crc8_calc(buf, byte_idx);
	printf("IC_CRC:%d\r\n",crc);
