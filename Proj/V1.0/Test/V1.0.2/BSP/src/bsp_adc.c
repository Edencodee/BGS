...
/* 记录当前DMA正在填充的缓冲区：true=A路，false=B路 */
static volatile bool s_isFillingA = false;
/* ------------------------------------------------------------------- */
...
{
	uint32_t freq = ADC_SAMPLING_FREQ;
	uint16_t TimerPeriod = (SystemCoreClock / freq ) - 1;
    ...
    timer_initpara.period            = TimerPeriod;
...
void DMA_Channel0_IRQHandler(void)
{
    if(dma_interrupt_flag_get(DMA_CH0, DMA_INT_FLAG_FTF))
    {
        ...
        ADC_StopSampling();
        /* DMA transfer complete,  allow EXTI to trigger the next sampling */
        g_isSampleDone = true;   
        /* Set completion flag based on the buffer filled this time */
        if(s_isFillingA) {
            g_isA_Done = true;
        } else {
            g_isB_Done = true;
...
void ADC_StartSampling(void)
{
    ...
    /* 清除 ADC EOC 标志，防止 RDATA 残留导致 DMA Enable 后立即
     * 响应挂起的旧请求，将上一轮最后一个转换结果写入新缓冲区 [0] */
...
{
    /* Update current filling buffer state before switching */
    s_isFillingA = (g_adcCaptureBuffer == (int16_t *)g_A_Buffer);
    dma_channel_disable(ADC_DMA_CHANNEL);
